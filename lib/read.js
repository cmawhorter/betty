...
export function isMissingError(err) {
  return err && typeof err.message === 'string' && err.message.indexOf('ENOENT') > -1;
}
...
export const kSourceFile = Symbol('source_file');
...
export async function readProjectFile(path, filename) {
  ...
  if (JSON_EXTS.indexOf(ext) > -1) {
    ...
    result = await evaluateJavascript(file);
  }
...
export async function evaluateJavascript(file) {
