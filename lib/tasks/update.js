...
export class LambdaUpdateTask extends OutputUpdateTask {
  ...
  async _processFunction(betty, region) {
    ...
    if (exists) {
      ...
      console.log('_processFunction; f');
  ...
  async _writeInlinePolicy(betty) {
    ...
    const policyDocument  = Policies.documentFromAssets(assets);
  ...
  async _writeManagedPolicy(betty) {
    ...
    const policyDocument = Policies.documentFromAssets(policy || []);
  ...
  async _before(betty) {
    ...
    console.log('loading service role');
    await this._loadServiceRole(betty);
    console.log('attaching basic exec');
    ...
    if (betty.resource.data.resources) {
      console.log('managed policies');
      await this._attachManagedPolicies(betty);
    }
    console.log('write service managed policy');
    await this._writeManagedPolicy(betty);
    console.log('done; write service managed policy');
  }

  async _run(betty) {
    const { resource } = betty;
    if (this.upload) {
      console.log('running _processFunction', resource.regions);
