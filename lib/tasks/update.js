...
export class LambdaUpdateTask extends OutputUpdateTask {
  ...
  async _attachExectionRole(awsAccountId, functionName, executionRole, rolesClient = Roles) {
    await rolesClient.attachAwsLambdaBasicExecutionRole(awsAccountId, functionName, executionRole);
  }
  ...
  async _before(betty) {
    ...
    await this._attachExectionRole(betty.context.awsAccountId, betty.resource.data.name, betty.context.awsLambdaRole);
    // inline policies
