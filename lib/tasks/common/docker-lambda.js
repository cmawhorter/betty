...
const _runDockerLambda_inheritEnvToArgs = inheritEnv => {
  const args = [];
  inheritEnv
    .forEach(name => args.push('-e', name));
  return args;
};
...
export function buildDockerRunArgs({
  ...
  return [
    ...
    ..._runDockerLambda_inheritEnvToArgs(inheritEnv),
...
export function runDockerLambda({
  ...
  let dockerTag;
  if (build) {
    ok(buildCommand, 'a build command is required when running build');
  ...
  return dockerRun(args, spawnOptions);
}
